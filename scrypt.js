...
const obs = new IntersectionObserver((entries) => {
  entries.forEach((entry, i) => {
    if (entry.isIntersecting) setTimeout(() => entry.target.classList.add('visible'), i * 72);
  });
}, { threshold: .08 });
...
function placeOrder() {
  ...
  const paymentLabel = payMethod.closest('label')?.innerText.replace('✓', '').trim() || 'Non precise';
  lastOrderSubject = `Commande ${orderNum} - ${prenom} ${nom}`;
  lastOrderMessage = buildOrderMessage(orderNum, { prenom, nom, tel, ville, adresse }, paymentLabel);
}
